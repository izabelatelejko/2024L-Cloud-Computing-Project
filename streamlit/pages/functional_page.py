...
def update_weather_fields():
    if input_lat != 0 and input_lon != 0:
        r_weather = requests.get(
            f"https://api.openweathermap.org/data/2.5/weather?lat={input_lat}&lon={input_lon}&appid={weather_api_key}"
        ).json()
...
urbanisation_dict = {"Rural": 0, "Midurban": 1, "Suburban": 2, "Urban": 3}
...
if not st.session_state["authentication_status"]:
    ...
    with st.popover("How to use PokéFinder?"):
        st.write(
            "First, accept the localisation prompt in your browser or input you longitude and latitude manually."
        )
        st.write(
            "Then, make sure you select the correct data for the following boxes and sliders."
        )
        st.write(
            "Now, only if you've already input latitude and longitude, press `Get more weather info for provided longitude and latitude`. It will fill the four boxes below with information about the places in coordinates you've provided."
        )
        st.write(
            "Once all the information is given, press `Load avaliable models` button to see available models. Depending on your tier, you might not be able to choose some of them. The selection button next to them will be disabled, but you'll still be able to see them."
        )
...
r = requests.get(f"https://api.sunrisesunset.io/json?lat={lat}&lng={lon}").json()[
    "results"
]
...
sunrise_time = datetime.strptime(
    f"{current_date} {r['sunrise']}", "%Y-%m-%d %I:%M:%S %p"
)
...
input_lat = st.number_input(label="Latitude", value=lat if lat is not None else 0)
input_lon = st.number_input(label="Longitude", value=lon if lon is not None else 0)
input_water = st.selectbox(
    label="Are you close (less than 100m) to water?", options=("Yes", "No")
)
input_terrain = st.selectbox(
    label="What kind of terrain are you in?", options=list(terrain_type_dict.keys())
)
input_urban = st.selectbox(
    label="How urbanised is the area you're in?", options=list(urbanisation_dict.keys())
)
input_poke_num = st.slider("How many Pokémon are near you?", 0, 15, 3)
input_pokestop_dist = st.slider(
    "How many km are you from the nearest pokestop?", 0.0, 20.0, 1.0, step=0.1
)
input_gym_dist = st.slider(
    "How many km are you from the nearest gym?", 0.0, 20.0, 1.0, step=0.1
)
pop_density = get_population_density(input_lat, input_lon)
input_density = st.number_input(
    "What's the estimated population density of where you are now?",
...
weather_button = st.button(
    "Get more weather info for provided longitude and latitude",
    on_click=update_weather_fields,
)
...
load_models_button = st.button(
    "Load available models",
    type="primary",
    disabled=st.session_state.disabled_load_model,
)
...
if st.session_state.clicked:
    for i in range(len(st.session_state.model_names)):
        ...
        with col3:
            if not (st.session_state.disable and "Gold" in model_name):
                ...
                if st.button(
                    "You need to be **gold** tier to access this model",
                    disabled=True,
                    key=model_name,
                ):
                    st.session_state.curr_model = model_name
    ...
    if st.session_state.curr_model != "":
        ...
        if st.button("Get prediction"):
            for i in range(len(st.session_state.model_names)):
                if str(st.session_state.model_names[i]) == (
        ...
        if st.session_state.predictions:
            if len(poke_ids) > 0:
                st.header(
                    f"The {len(poke_ids)} Pokémon most likely to appear near you are:"
                )

            pokemon_data = pd.read_csv("pokemon_urls_names.csv")
            poke_data = pokemon_data[pokemon_data["id"].isin(poke_ids)]
            poke_data["id"] = pd.Categorical(
                poke_data["id"], categories=poke_ids, ordered=True
            )
            ...
            for id in poke_ids:
                images.append(
                    "poke_img/"
                    + poke_data[poke_data["id"] == id]["name"].values[0]
                    + ".jpg"
                )
